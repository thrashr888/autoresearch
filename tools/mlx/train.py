...
def env_float_tuple(name, default):
    ...
    if len(parts) != len(default):
        raise ValueError(f"{name} expected {len(default)} comma-separated values, got {value!r}")
...
# Model architecture
ASPECT_RATIO = env_int("ASPECT_RATIO", 64)
HEAD_DIM = env_int("HEAD_DIM", 128)
WINDOW_PATTERN = env_str("WINDOW_PATTERN", "SSSL")

# v0.1: AdamW only. Muon port is future work.
TOTAL_BATCH_SIZE = env_int("TOTAL_BATCH_SIZE", 2**16)
EMBEDDING_LR = env_float("EMBEDDING_LR", 0.6)
UNEMBEDDING_LR = env_float("UNEMBEDDING_LR", 0.004)
MATRIX_LR = env_float("MATRIX_LR", 0.04)
SCALAR_LR = env_float("SCALAR_LR", 0.5)
WEIGHT_DECAY = env_float("WEIGHT_DECAY", 0.2)
ADAM_BETAS = env_float_tuple("ADAM_BETAS", (0.8, 0.95))
WARMUP_RATIO = env_float("WARMUP_RATIO", 0.0)
WARMDOWN_RATIO = env_float("WARMDOWN_RATIO", 0.5)
FINAL_LR_FRAC = env_float("FINAL_LR_FRAC", 0.0)

# Model size
DEPTH = env_int("DEPTH", 4)
DEVICE_BATCH_SIZE = env_int("DEVICE_BATCH_SIZE", 16)
FINAL_EVAL_BATCH_SIZE = env_int("FINAL_EVAL_BATCH_SIZE", 256)
STARTUP_EXCLUDE_STEPS = env_int("STARTUP_EXCLUDE_STEPS", 1)

# Run controls
TIME_BUDGET_SECONDS = env_int("TIME_BUDGET_SECONDS", TIME_BUDGET)
HEARTBEAT_SECS = env_float("HEARTBEAT_SECS", 30.0)
QUICK_EVAL_INTERVAL_SECS = env_float("QUICK_EVAL_INTERVAL_SECS", 0.0)
QUICK_EVAL_TOKENS = env_int("QUICK_EVAL_TOKENS", 65536)
QUICK_EVAL_BATCH_SIZE = env_int("QUICK_EVAL_BATCH_SIZE", 128)
RUN_NAME = env_str("RUN_NAME", "")
RUN_DIR = env_str("RUN_DIR", "")
...
if RUN_DIR:
    ...
    progress_log = open(os.path.join(RUN_DIR, "progress.jsonl"), "a", encoding="utf-8")
...
config_summary = {
    ...
    "quick_eval_interval_secs": QUICK_EVAL_INTERVAL_SECS,
    "quick_eval_tokens": QUICK_EVAL_TOKENS,
    "quick_eval_batch_size": QUICK_EVAL_BATCH_SIZE,
}
print("Config:")
print(json.dumps(config_summary, indent=2, sort_keys=True))
if progress_log is not None:
    progress_log.write(json.dumps({"event": "config", **config_summary}) + "\n")
    progress_log.flush()
...
last_quick_eval_wall = time.time()
last_quick_val_bpb = None
...
while True:
    ...
    if last_quick_val_bpb is not None:
        status += f" | quick_val_bpb: {last_quick_val_bpb:.6f}"
    status += f" | peak_vram_mb: {peak_vram_mb:.1f}"

    print(f"\r{status}    ", end="", flush=True)

    now = time.time()
    heartbeat_due = HEARTBEAT_SECS > 0 and (now - last_heartbeat_wall) >= HEARTBEAT_SECS
    quick_eval_due = (
        QUICK_EVAL_INTERVAL_SECS > 0
        and step >= STARTUP_EXCLUDE_STEPS
        and (now - last_quick_eval_wall) >= QUICK_EVAL_INTERVAL_SECS
    )

    if quick_eval_due:
        print()
        quick_eval_started = time.time()
        print(
            f"quick_eval start | step {step:05d} | eval_tokens: {QUICK_EVAL_TOKENS} | "
            f"batch_size: {QUICK_EVAL_BATCH_SIZE}",
            flush=True,
        )
        last_quick_val_bpb = evaluate_bpb(
            model,
            tokenizer,
            QUICK_EVAL_BATCH_SIZE,
            eval_tokens=QUICK_EVAL_TOKENS,
        )
        quick_eval_seconds = time.time() - quick_eval_started
        print(
            f"quick_eval done  | step {step:05d} | val_bpb: {last_quick_val_bpb:.6f} | "
            f"seconds: {quick_eval_seconds:.1f}",
            flush=True,
        )
        last_quick_eval_wall = time.time()
        last_heartbeat_wall = last_quick_eval_wall
        heartbeat_due = False
    ...
    if heartbeat_due:
        ...
        if last_quick_val_bpb is not None:
            heartbeat["quick_val_bpb"] = round(last_quick_val_bpb, 6)
        print()
        print(f"heartbeat | {json.dumps(heartbeat, sort_keys=True)}", flush=True)
        last_heartbeat_wall = time.time()
            ...
            json.dumps(
                {
                    ...
                    "quick_val_bpb": last_quick_val_bpb,
                }
            )
            + "\n"
        )
    ...
    step += 1
    if step >= STARTUP_EXCLUDE_STEPS and total_training_time >= TIME_BUDGET_SECONDS:
        break
